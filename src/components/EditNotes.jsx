    ...
    <div className='w-full h-full lg:overflow-y-auto flex pb-10'>
        <div className="flex-1 border border-neutral-200">
            {
                ...
                (
                    ...
                    <NavLink to={"/"} className='text-base font-medium focus:outline-none mt-2 sm:mt-0 px-4 py-2 rounded-md bg-blue-500 text-primary-foreground'>Return to Home</NavLink>
                    </div>
                ) :
                (
                    <>
                        <div className='flex justify-between items-center px-6 pt-2 lg:hidden'>
                            ...
                            <button className='border border-neutral-300/65 rounded-sm py-2 px-3.5' onClick={() =>setShowDeleteWarning(true)}>
                                <img src={icons.trash} alt="trash-icon" className='w-4'/>
                            </button>
                            <button className='border border-neutral-300/65 rounded-sm py-2 px-3.5' onClick={() =>setShowArchiveWarning(true)}>
        ...
        <div className={`fixed top-0 bottom-0 left-0 right-0 bg-black/60 z-20 ${showDeleteWarning ? "" : "hidden"}`}>
            ...
            <div className='flex flex-col-reverse space-x-2 sm:flex-row sm:justify-end sm:space-y-2'>
                <button className='text-base font-medium hover:cursor-pointer focus:outline-none mt-2 sm:mt-0 bg-neutral-100 px-4 py-2 rounded-md' onClick={() => setShowDeleteWarning(false)} >Cancel</button>
                <button className='text-base font-medium hover:cursor-pointer focus:outline-none mt-2 sm:mt-0 px-4 py-2 rounded-md bg-red-500 text-primary-foreground' onClick={() => deleteHandler(noteId)}>Delete Note</button>
            </div>
            </div>
        </div>
        ...
        <div className={`fixed top-0 bottom-0 left-0 right-0 bg-black/60 z-20 ${showArchiveWarning ? "" : "hidden"}`}>
            ...
            <div className='flex flex-col-reverse space-x-2 sm:flex-row sm:justify-end sm:space-y-2'>
                <button className='text-base font-medium hover:cursor-pointer focus:outline-none mt-2 sm:mt-0 bg-neutral-100 px-4 py-2 rounded-md' onClick={() => setShowArchiveWarning(false)} >Cancel</button>
                <button className='text-base font-medium hover:cursor-pointer focus:outline-none mt-2 sm:mt-0 px-4 py-2 rounded-md bg-blue-500 text-primary-foreground' onClick={() => archiveHandler( setShowArchiveWarning, archiveStatus )}>
                {
                    archiveStatus? "Unarchive Note" : "Archive Note"
                }
                </button>
